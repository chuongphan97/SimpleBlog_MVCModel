...
<div class="container">
    <div class="row">
        ...
        <div class="col-lg-8">
            <!-- Featured blog post-->
            <c:choose>
                <c:when test="${adPost.image == null}">
                    <div class="card mb-4">
                        <a href="#!"><img class="card-img-top" src="https://dummyimage.com/850x350/dee2e6/6c757d.jpg" alt="..." style="max-width: 850px; max-height: 350px" /></a>
                        <div class="card-body">
                            <div class="small text-muted">${adPost.date} by <a href="/blogs?action=wall&username=${adPost.username}">${adPost.username}</a></div>
                            <h2 class="card-title">${adPost.title}</h2>
                            <p class="card-text post-content">${adPost.content}</p>
                            <a class="btn btn-primary" href="/blogs?action=detail-post&post_id=${adPost.post_id}">Read more →</a>
                        </div>
                    </div>
                </c:when>
                <c:otherwise>
                    <div class="card mb-4">
                        <a href="#!"><img class="card-img-top" src="${adPost.image}" alt="..." style="max-width: 850px; max-height: 350px" /></a>
                        <div class="card-body">
                            <div class="small text-muted">${adPost.date} by <a href="/blogs?action=wall&username=${adPost.username}">${adPost.username}</a></div>
                            <h2 class="card-title">${adPost.title}</h2>
                            <p class="card-text post-content">${adPost.content}</p>
                    ...
                    <div class='panel-group'>
                        <div class='panel panel-default'>
                            <div class="panel-body">
                                <form method="post" action="/blogs?action=add">
                                    <fieldset>
                                        <legend>Add new post</legend>
                                        <div class='form-group' hidden >
                                            <label class='control-label'>Username</label>

                                            <input class='form-control' placeholder='Enter username' name='username' type='text'  value="${account.getUsername()}">
                                        </div>
                                        <div class='form-group'>
                                            <input required class='form-control' placeholder='Enter title' type='text' name="title">
                                        </div>
            ...
            <div class="row">
                <c:forEach items="${posts}" begin="0" end="7" var="p">
                    <c:if test="${p.image == null}">
                        <div class="col-lg-6">
                            <div class="card mb-4">
                                ...
                                <div class="card-body">
                                    <div class="small text-muted">${p.date} by <a href="/blogs?action=wall&username=${p.username}">${p.username}</a></div>
                ...
                <div class="card-body">
                    <div class="input-group">
                        <form action="/blogs?action=search" method="post">
                            <input required name="search" class="form-control" type="text" placeholder="Enter search term..." aria-label="Enter search term..." aria-describedby="button-search" />
